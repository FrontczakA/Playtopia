...
public interface GameRepository extends JpaRepository<Game, Long> {
    @Query("SELECT g FROM Game g WHERE (:min IS NULL OR g.price > :min) AND (:max IS NULL OR g.price < :max) AND (:console IS NULL OR g.platform = :console) AND (:category IS NULL OR g.genre = :category)")
